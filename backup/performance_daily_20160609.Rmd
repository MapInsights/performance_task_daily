...
df$BlockHour <-as.numeric(format( as.POSIXct(df$from), "%H"))
    ...
    ggplot(x, aes(x = BlockHour, y = Block)) +
    geom_bar(position = 'stack',stat="identity", fill = 'blue')  +
    ggtitle('G10 blocks') +
    xlab('Hour') +
    ylab('0 = no block, 1 = block') +
...
### Yesterday jobs up to 5 min late in %% 

```{r echo= FALSE,fig.width=18, fig.height=10,warning=FALSE, message= FALSE}
## 5 min late

# calculate performance ontime
performance_5min<-gtc[gtc$arrival_bucket == '20+ min early'
              |  gtc$arrival_bucket == '10-20 min early'
              |  gtc$arrival_bucket == '6-10 min early'
              |  gtc$arrival_bucket == '4-5 min early'
              |  gtc$arrival_bucket == '1-3 min early'
              ...
              |  gtc$arrival_bucket == '4-5 min late',]
...
df_performance_5min <- group_by(performance_5min, Date) %>%
  summarise(JobSum = sum(Count))
df_performance_5min <- merge(df_performance_5min, df_per_date, by = c('Date') )
df_performance_5min$Percent <- round((df_performance_5min$JobSum / df_performance_5min$JobTotal)*100,2)

df_performance_5min$Percent
```

### Day shift. Jobs up to 5 min late in %% 

```{r echo= FALSE,fig.width=18, fig.height=10,warning=FALSE, message= FALSE}
## 5 min late

# calculate performance ontime
performance_5min<-dayshift[dayshift$arrival_bucket == '20+ min early'
              |  dayshift$arrival_bucket == '10-20 min early'
              |  dayshift$arrival_bucket == '6-10 min early'
              |  dayshift$arrival_bucket == '4-5 min early'
              |  dayshift$arrival_bucket == '1-3 min early'
              ...
              |  dayshift$arrival_bucket == '4-5 min late',]
...
### Night shift. Jobs up to 5 min late in %% 

```{r echo= FALSE,fig.width=18, fig.height=10,warning=FALSE, message= FALSE}
## 5 min late

# calculate performance ontime
performance_5min<-nightshift[nightshift$arrival_bucket == '20+ min early'
              |  nightshift$arrival_bucket == '10-20 min early'
              |  nightshift$arrival_bucket == '6-10 min early'
              |  nightshift$arrival_bucket == '4-5 min early'
              |  nightshift$arrival_bucket == '1-3 min early'
              ...
              |  nightshift$arrival_bucket == '4-5 min late',]
...
drivers$Timestamp <- as.POSIXct(drivers$Timestamp)
 ...
 # sum by GTC and GTLite for each 5 min
...
### Performance (on time / 5 min late)
...
df_performance_5min <- group_by(performance_5min, Date, Weekday, Hour, Type) %>%
  summarise(JobSum = sum(Count))
df_performance_5min <- merge(df_performance_5min, df_per_date, by = c('Date','Hour', 'Type') )
df_performance_5min$Percent <- round((df_performance_5min$JobSum / df_performance_5min$JobTotal)*100,2)




df_performance_5min$Percent_bucket <- cut(
  df_performance_5min$Percent, breaks = c(0,75,95,100)
)




ggplot(df_performance_5min, aes(x = Hour, y = Percent, fill = Percent_bucket)) +
    geom_bar(position = 'stack', stat='identity') +
    facet_wrap(~Type) +
    ggtitle('Performance (inc. arrival 5 min late)') +
    theme(text = element_text(size=16)) +
    geom_text(aes(label = Percent), size = 4)
...
df_performance_5min <- group_by(performance_5min, Date, Weekday, Hour, Grade) %>%
  summarise(JobSum = sum(Count))
df_performance_5min <- merge(df_performance_5min, df_per_date, by = c('Date','Hour','Grade') )
df_performance_5min$Percent <- round((df_performance_5min$JobSum / df_performance_5min$JobTotal)*100,2)
...
ggplot(df_performance_5min, aes(x = Hour, y = Percent, fill = Percent_bucket)) +
    geom_bar(position = 'stack', stat='identity') +
    facet_wrap(~Grade) +
    ggtitle('Performance by Grade (inc. arrival 5 min late)') +
    theme(text = element_text(size=16))
```
...
df_performance_5min <- group_by(performance_5min, Date, Weekday, Hour, Type2) %>%
  summarise(JobSum = sum(Count))
df_performance_5min <- merge(df_performance_5min, df_per_date, by = c('Date','Hour','Type2') )
df_performance_5min$Percent <- round((df_performance_5min$JobSum / df_performance_5min$JobTotal)*100,2)
...
ggplot(df_performance_5min, aes(x = Hour, y = Percent, fill = Percent_bucket)) +
    geom_bar(position = 'stack', stat='identity') +
    facet_wrap(~Type2) +
    ggtitle('Performance by Type (inc. arrival 5 min late)') +
    theme(text = element_text(size=16))
