...
```{r echo=FALSE}
start <- as.Date(Sys.time() - (3*60*60*24))
start
```
...
declare @TO datetime
set @FROM = DATEADD(DAY, -3, CONVERT(CHAR(10), getdate(), 111))
set @TO = DATEADD(DAY, -3, CONVERT(CHAR(10), getdate(), 111)) +'23:59:59'
...
(select top 1 jh.shortArg from archive_echo_core_prod.dbo.job_history jh where jh.jobReference=j.id and jh.shortArg like '%about Late to Pickup') as 'NotificationLate',
(select top 1 username from archive_echo_core_prod..job_history where actionType=18 and jobReference=j.id) as 'AllocatedBy',
...
declare @FROM datetime
declare @TO datetime
set @FROM = DATEADD(DAY, -4, CONVERT(CHAR(10), getdate(), 111))
set @TO = DATEADD(DAY, -3, CONVERT(CHAR(10), getdate(), 111)) +'23:59:59'
...
declare @FROM datetime
declare @TO datetime
set @FROM = DATEADD(DAY, -8, CONVERT(CHAR(10), getdate(), 111))
set @TO = DATEADD(DAY, -3, CONVERT(CHAR(10), getdate(), 111)) +'23:59:59'
...
yesterday <- weekdays(Sys.time() - (3*24*60*60))
...
cb_from_to$from <- paste(as.Date(Sys.time() - (3*24*60*60)), cb_from_to$from)
cb_from_to$to <- paste(as.Date(Sys.time() - (3*24*60*60)), cb_from_to$to)
...
dropbox<-c("total",dropbox,sum(gtc2$totalPrice,na.rm=TRUE))

revenue<-group_by(gtc2,Type) %>% summarise(Jobs=sum(Count,na.rm=TRUE),performance=0,performance1=0,Revenue=sum(totalPrice,na.rm=TRUE))
dropbox<-rbind(dropbox,revenue)
write.csv(dropbox,file=dropname)
...
file <- jobhistory
yesterday <- as.Date(Sys.time()-(3*60*60*24))
...
#Jobs and Drivers by 15 minutes graph
```{r echo= FALSE,fig.width=18, fig.height=10,warning=FALSE, message= FALSE}
...
#now do the graph
 ...
 ggplot(gtc_per_15min  ) +
  ...
  # theme(panel.grid.major=element()) +
...
Ratio<-round(DeadMilesSum/JobMilesSum*100,2)

tbl2<-rbind(c("Dead Miles","Job Miles","Dead Mile %"),c(round(DeadMilesSum,0),round(JobMilesSum,0),Ratio))
