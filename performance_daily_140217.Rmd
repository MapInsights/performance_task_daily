...
```{r echo=FALSE}
start <- as.Date(Sys.time() - (60*60*24)*6)
start
```
...
set @TO = DATEADD(DAY, -6, CONVERT(CHAR(10), getdate(), 111)) +'23:59:59'
...
(select top 1 jh.shortArg from archive_echo_core_prod..job_history jh where jh.jobReference=j.id and jh.shortArg like '%about Late to Pickup') as 'NotificationLate',
...
declare @FROM datetime
declare @TO datetime
set @FROM = DATEADD(DAY, -7, CONVERT(CHAR(10), getdate(), 111))
set @TO = DATEADD(DAY, -6, CONVERT(CHAR(10), getdate(), 111)) +'23:59:59'
...
set @endDate = DATEADD(DAY, -6, getdate()) + '23:59:59'
...
declare @FROM datetime
declare @TO datetime
set @FROM = DATEADD(DAY, -12, CONVERT(CHAR(10), getdate(), 111))
set @TO = DATEADD(DAY, -6, CONVERT(CHAR(10), getdate(), 111)) +'23:59:59'
...
set @yesterdayFROM = DATEADD(DAY, -6, CONVERT(CHAR(10), getdate(), 111))
set @yesterdayTO = DATEADD(DAY, -6, CONVERT(CHAR(10), getdate(), 111)) + '23:59:59'
...
file <- jobhistory
yesterday <- as.Date(Sys.time()-(60*60*24)*6)
...
gtc[gtc$name %in% c('Exec', 'Saloon - Executive'),]$Service <- 'Executive'
gtc[gtc$name %in% c('MPV - 6 seater', 'MPV - 8 Seater', 'MPV - Corporate'),]$Service <- 'MPV'
...
declare @yesterdayFROM datetime
declare @yesterdayTO datetime
set @yesterdayFROM =DATEADD(DAY, -2, CONVERT(CHAR(10), getdate(), 111))
set @yesterdayTO =DATEADD(DAY, -2, CONVERT(CHAR(10), getdate(), 111)) + '23:59:59'
...
handbacks_platform2<-group_by(handbacks,creationType, result, shift)%>% summarise(total =sum(Count))
