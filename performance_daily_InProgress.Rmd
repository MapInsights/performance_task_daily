...
f1$OnTime3Mins<-as.numeric(f1$OnTime3Mins)
f1$OnTime<-as.numeric(f1$OnTime)
f1$ActualClassic<-as.numeric(f1$ActualClassic)
f1$ActualLite<-as.numeric(f1$ActualLite)
...
ActiveJobs<-sqlQuery( odbcChannel,"
declare @startDate date
declare @endDate date
set @startDate = DATEADD(DAY, -1, getdate())
set @endDate = DATEADD(DAY, -1, getdate()) + '23:59:59'
select distinct j.id,j.jobDate ,jh0.actionDate 'Accepted',
--jh.actionDate 'Arrived' ,
jh1.actionDate 'POB',jh2.actionDate 'POD',DATEDIFF(minute,jh0.actionDate,jh2.actionDate)
from echo_core_prod..jobs j
left join echo_core_prod..job_history jh0 on j.id = jh0.jobReference and jh0.shortArg	= 'Accepted'
--left join job_history jh on j.id = jh.jobReference and jh.shortArg	= 'Arrived'
left join echo_core_prod..job_history jh1 on j.id = jh1.jobReference and jh1.shortArg	= 'POB'
left join echo_core_prod..job_history jh2 on j.id = jh2.jobReference and jh2.shortArg	= 'POD'
...
write.xlsx2(df_by_arrival, file=filename, sheetName="ArrivalsDetailed", append=TRUE,row.names = FALSE)
...
write.xlsx2(df_by_arrival, file=filename, sheetName="ArrivalsGrade", append=TRUE,row.names = FALSE)
...
ggplot(ResponsePostCode, aes(x = Hour, y = AvgResponseTime, fill = pickupAddress)) +
...
```{r echo= FALSE,fig.width=18, fig.height=10,warning=FALSE, message= FALSE}
TimeTable<-as.data.frame(seq.POSIXt(as.POSIXlt(start),as.POSIXlt(start + 1),by = "5 min",tz="GMT"))

TimeTable$Accepted<-0
TimeTable$POB<-0
TimeTable$POD<-0
ActiveJobs$Count<-1
...
for  ( i in c(1:289) ){
 TimeTable$Accepted[i]<-sum(ActiveJobs$Accepted<=TimeTable[i,1],na.rm = TRUE)
  TimeTable$POB[i]<-sum(ActiveJobs$POB<=TimeTable[i,1],na.rm = TRUE)
 TimeTable$POD[i]<-sum(ActiveJobs$POD<=TimeTable[i,1],na.rm = TRUE)
}
