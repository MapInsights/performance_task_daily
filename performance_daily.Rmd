...
# valueBox(asa_daily, 
#          icon = "fa-comments",
#          color = ifelse(asa_daily > 10, "warning", "primary"))
```
...
# valueBox(ART, 
#          icon = "fa-comments",
#          color = ifelse(ART >2, "warning", "primary"))
```
...
# valueBox(AHT_daily, 
#          icon = "fa-comments",
#          color = ifelse(AHT_daily > 140, "warning", "primary"))
```



### Abandoned, Over30, Under30 and ASA
...
asa_per_hour$Resolution_ <- 'asa'

write.xlsx(asa_per_hour, file=filename, sheetName="asa", append = TRUE) 


# volume per Half hour 30 sec and factor  'Abd', 'Under30' , 'Over30'
volume_per_hour_Abd_UO30 <- as.data.frame(group_by(cc, Hour, Resolution_)  %>% 
                                         summarise(call_sum = sum(Count)))

write.xlsx(volume_per_hour_Abd_UO30, file=filename, sheetName="volume_per_hour_Abd_UO30", append = TRUE) 

Palette22 <- c("#FF0000", "#000000", "#FFCC99","#3399FF")

volume_per_hour_Abd_UO30$Resolution_ <- factor(volume_per_hour_Abd_UO30$Resolution_,levels= c('Abd', 'Over30', 'Under30'))

ggplot(data = volume_per_hour_Abd_UO30, aes(x=Hour, y=call_sum,fill = Resolution_, group = Resolution_))+
  geom_bar(position = 'stack', stat='identity') +
  geom_line(data =asa_per_hour, aes(x = Hour, y = asa), color = 'black', size =1) +
  ggtitle('') +
  xlab('Hour') +
  ylab('Number of Calls') + scale_fill_manual(values=Palette22) +
...
#### Q and A split of calls per 1/2 hour
...
ggplot(volume_per_hour)+
  ...
  ggtitle('black line: agent logon') +
  xlab('Hour') +
