...
select
	...
	case when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 0 and DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate)< 388) then 'NightShift'
	     when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 388 and DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate)< 1108) then 'DayShift'
	     when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 1108) then 'NightShift' end 'Shift'
from echo_core_prod..job_history  jh
...
gtc[gtc$TimeInMin >= 388 & gtc$TimeInMin <= 1108,]$shift <- 'DayShift'
```
