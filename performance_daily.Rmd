...
daysAgo<-1
start <- as.Date(Sys.time() - (60*60*24*daysAgo))
start
...
xlToday<-as.Date(start+1)
xlToday<-difftime(xlToday,'1900-01-01',units='days') + 2 
xlToday<-as.numeric(xlToday)
rowNo<-as.numeric(difftime(as.Date(start+1),'2015-12-21',units='days') )
...
gtc  <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
set @TO =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111)) + '23:59:59'
...
DeadMiles<- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
CRMQuery <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
drivers <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
jobhistory <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo+1,", CONVERT(CHAR(10), getdate(), 111))
set @TO =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111)) + '23:59:59'
...
ResponseTime <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo+1,", CONVERT(CHAR(10), getdate(), 111))
...
cashblock <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo+5,", CONVERT(CHAR(10), getdate(), 111))
set @TO =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111)) + '23:59:59'
...
bookings <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
select j.id, j.secondNumber, jh.userName, jh.actionDate,jh.shortArg, jh.jobReference,jh2.shortArg,jh2.actionDate,jh2.userName,j.creationType,j.jobStatus,j.creationDate,
datediff(minute,j.creationDate, jh.actionDate) 'minBeforeAction',
...
snapshot<- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
where j.jobDate between @FROM and @TO"))


snapshotASAP<- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
portal<- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
select
	...
	case when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 0 and DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate)< 37.5) then 'NightShift'
	     when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 375 and DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate)< 1095) then 'DayShift'
	     when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 1095) then 'NightShift' end 'Shift'
from echo_core_prod..job_history  jh
...
where jh.actionType=19 and jh.userName=i.fullName and js.orderNumber=0 and j.jobDate between @FROM and @TO
union
select
    jh.jobReference as 'Job_number',
    j.jobstatus as 'job status',
    ...
    ar.name as 'PostCodePU',
    DATEPART(hh, j.jobDate)*60 + DATEPART(mi, j.jobDate) 'TimeInMin',
	case when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 0 and DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate)< 375) then 'NightShift'
	     when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 375 and DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate)< 1095) then 'DayShift'
	     when (DATEPART(hour, j.jobDate)*60 + DATEPART(mi, j.jobDate) >= 1095) then 'NightShift' end 'Shift'
...
```{r}
match <- c('2200', '6443', 'G1', 'G2', 'G3', 'G4', 'G5', 'G6', 'G7', 'G8', 'G9.1', 'G9.5', 'GTC888', 'G50', 'G51', 'G5555', '7002', 'LONGTC1387', 'C2449')
...
```{r}
#Day defined as 6:15 to 18:14 
#Night defined as 00:00 to 06:14 and 18:15 to 23:59
gtc$shift <- 'NightShift'
gtc[gtc$TimeInMin >= 375 & gtc$TimeInMin <= 1095,]$shift <- 'DayShift'
```


```{r}
### Day shift (6:15 to 18:14). Job Volume  
...
```{r}
### Night shift (00:00 to 06:14 and 18:15 to 23:59). Job Volume  
...
write.xlsx2(gtc, file=filename, sheetName="performance_jobs",row.names = FALSE) 
```
...
# fix bug if hours are missing
DRIVERS$Hour = as.numeric(substr(DRIVERS$Timestamp, 12,13))


write.xlsx2(DRIVERS, file=filename, sheetName="Drivers",row.names = FALSE, append = TRUE) 
...
file <- jobhistory
yesterday <-  as.Date(Sys.time()-(60*60*24*daysAgo))
...
# secude your code is no late account asaps
if ('asap_accounts' %in% accounts_arrival$Type2) {
...
#Get latest week always
rowNow<-as.numeric(difftime(as.Date(Sys.Date()),'2015-12-21',units='days') )

rowStart<-rowNow-8
lastweek<-f1[c(rowStart:rowNow),c(1:2,4:25)]
...
OneTBookings[(OneTBookings$jobStatus==9 & OneTBookings$userName!="OneTransport"
              & OneTBookings$minBeforeAction > 3)  ,"result"]<-"Handback"
...
calls  <- sqlQuery(odbcChannel,paste(
                    "
declare @FROM datetime
declare @TO datetime
set @FROM =DATEADD(DAY, -",daysAgo,", CONVERT(CHAR(10), getdate(), 111))
...
and ac.Date between @FROM and @TO
"))
odbcClose(odbcChannel)
...
#Subbed jobs
###################################################################
subbedJobs<-nrow(gtc[!is.na(gtc$partner_id), ])

inputA<-c(inputA,subbedJobs)
...
#time under 20 mins
###################################################################
inputA<-c(inputA,day_under20,night_under20,total_under20)
