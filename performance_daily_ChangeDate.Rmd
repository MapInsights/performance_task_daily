...
```{r echo=FALSE, message=FALSE,warning=FALSE}
forecast<-'forecast/F2_dailyJobsForecast_Values.xlsx'
...
--	adr1.name as 'DropOffPC'  ,		
	...
	left join 		
		(select	
			...
			from archive_echo_core_prod..autoallocations a
			group by a.jobId)a1 
	...
	left join 		
		(Select 	
			...
			journeyToPU,
			dispatchTime 
			from archive_echo_core_prod..autoallocations)a2
...
from (select * from echo_core_prod..dynamic_delay_promises union select * from archive_echo_core_prod..dynamic_delay_promises) ddp where ddp.source = 8  and
 startDate between  @from and @to                    
                     
                     "))

cashblock <- sqlQuery( odbcChannel, paste(open,
"
...
bookings <-sqlQuery( odbcChannel, paste(open,
  "
...
snapshot<-sqlQuery( odbcChannel, paste(open,
"
...
snapshotASAP<-sqlQuery( odbcChannel,paste(open,
"
select --distinct datepart(HOUR,j.jobdate) 'Hour',
		count(DISTINCT (case when  datepart(hour,jt.actualAcceptTime) < '3' and datepart(hour,jt.actualPodTime) >= '3' then jt.job_Id else null end)) '3am',
		count(DISTINCT (case when  datepart(hour,jt.actualAcceptTime) < '8' and datepart(hour,jt.actualPodTime) >= '8' then jt.job_Id else null end)) '8am',
...
OneTBookings<-bookings[bookings$creationType ==5 & bookings$JobYear=="2016",]
...
CabfindBookings<-bookings[bookings$creationType ==18 & bookings$JobYear=="2016",]
...
MSBookings<-bookings[bookings$creationType ==4 & bookings$JobYear=="2016",]
...
CityFleetBookings<-bookings[bookings$creationType ==16 & bookings$JobYear=="2016",]
